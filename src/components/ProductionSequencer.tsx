...
import { Play, RotateCcw, Settings2, BarChart3, Clock, Gauge, TrendingDown, CheckCircle2, Trash2, Plus, ArrowDownToLine, LayoutList, Package, RefreshCw, Download } from 'lucide-react';
...
const ScenarioTradeoffChart: React.FC<{ scenarios: any[], activeId?: string, onSelect?: (id: any) => void }> = ({ scenarios, activeId, onSelect }) => {
    ...
    const data = scenarios
        ...
        .map(s => {
            ...
            return {
                ...
                total,
                // Raw physical values
                toneladasPerdidas: res.ventaPerdidaTotal || 0,    // Tn
                horasCambio: res.tiempoTotalCambio || 0,           // h
            };
        });
    ...
    const fmt = (n: number, dec = 0) => n.toLocaleString(undefined, { maximumFractionDigits: dec, minimumFractionDigits: dec });
            ...
            <div className="relative h-72 w-full flex items-end justify-around px-2 md:px-12 pt-10">
                ...
                {data.map((d) => {
                    ...
                    return (
                        <div
                            key={d.id}
                            className="flex-1 flex flex-col items-center group relative h-full justify-end max-w-[200px] gap-6"
                            onMouseEnter={() => setHoveredId(d.id)}
                            onMouseLeave={() => setHoveredId(null)}
                        >
                            {/* Tooltip */}
                            {isHovered && (
                                <div className="absolute bottom-full mb-6 z-50 bg-slate-900 text-white rounded-2xl shadow-2xl p-4 w-56 pointer-events-none animate-in fade-in zoom-in-95 duration-150">
                                    {/* Arrow */}
                                    <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-2 overflow-hidden">
                                        <div className="w-3 h-3 bg-slate-900 rotate-45 translate-y-[-6px] translate-x-[2px]"></div>
                                    </div>

                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 text-center">{d.label}</p>

                                    {/* Blue: Lost Sales */}
                                    <div className="flex items-start gap-3 mb-3 bg-blue-900/30 rounded-xl p-2.5">
                                        <div className="w-2.5 h-2.5 rounded-full bg-blue-400 mt-0.5 shrink-0 shadow-sm shadow-blue-400/50"></div>
                                        <div className="flex-1">
                                            <p className="text-[9px] font-black uppercase tracking-wider text-blue-300 mb-1">Venta Perdida</p>
                                            <p className="text-sm font-black text-white leading-none">
                                                {fmt(d.toneladasPerdidas, 2)} <span className="text-[10px] font-bold text-blue-300">Tn</span>
                                            </p>
                                            <p className="text-[11px] font-bold text-blue-300 mt-0.5">{fmtUSD(d.cvp)}</p>
                                        </div>
                                    </div>

                                    {/* Orange: Changeover */}
                                    <div className="flex items-start gap-3 bg-orange-900/30 rounded-xl p-2.5">
                                        <div className="w-2.5 h-2.5 rounded-full bg-orange-400 mt-0.5 shrink-0 shadow-sm shadow-orange-400/50"></div>
                                        <div className="flex-1">
                                            <p className="text-[9px] font-black uppercase tracking-wider text-orange-300 mb-1">Tiempo de Cambio</p>
                                            <p className="text-sm font-black text-white leading-none">
                                                {fmt(d.horasCambio, 1)} <span className="text-[10px] font-bold text-orange-300">h</span>
                                            </p>
                                            <p className="text-[11px] font-bold text-orange-300 mt-0.5">{fmtUSD(d.ctc)}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-3 pt-3 border-t border-slate-700 flex justify-between items-center">
                                        ...
                                        <span className="text-sm font-black text-white">{fmtUSD(d.total)}</span>
                                    </div>
                                </div>
                            )}

                            <div
                                className={`w-20 md:w-28 relative flex flex-col justify-end transition-all duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:scale-110 group-hover:-translate-y-2 cursor-pointer ${isActive ? 'scale-105 ring-4 ring-blue-500/20 rounded-b-xl' : 'opacity-80'}`}
                            ...
                            <div className="flex flex-col items-center gap-2">
                                <span className={`text-[11px] font-black uppercase tracking-tighter text-center transition-colors duration-300 ${isActive ? 'text-blue-700' : 'text-slate-400 group-hover:text-slate-600'}`}>{d.label}</span>
                                {isActive && <div className="w-8 h-1 bg-blue-600 rounded-full shadow-lg shadow-blue-200 animate-in zoom-in slide-in-from-bottom-2"></div>}
                            </div>
                        </div>
                    );
                })}
...
export const ProductionSequencer: React.FC = () => {
    ...
    const handlePaste = (e: React.ClipboardEvent) => {
        ...
        const newDraftItems: DraftItem[] = rows.map((row) => {
            ...
            // Match with master articles (STRICTLY ACTIVE PROCESS ONLY)
            const art = articles.find(a =>
                String(a.codigoProgramacion || '').trim() === rawSku ||
                String(a.skuLaminacion || '').trim() === rawSku
            );

            // Match with SAP data (GLOBAL DATA - for stock/sales info)
            ...
            return {
                ...
                skuCode: rawSku,
                description: art?.descripcion || sapItem?.descripcion || 'SKU No Encontrado en Maestro',
                quantity: cleanQty,
    ...
    // Re-mapear el borrador contra el maestro actual del laminador (corrige S/N en ID Cambio)
    const handleRefreshDraft = () => {
        if (articles.length === 0) {
            alert('El Maestro de Artículos está vacío. Importa los artículos del laminador primero.');
            return;
        }
        let updated = 0;
        const refreshed = draftItems.map(item => {
            const art = articles.find(a =>
                String(a.codigoProgramacion || '').trim() === item.skuCode ||
                String(a.skuLaminacion || '').trim() === item.skuCode
            );
            if (art) {
                updated++;
                return {
                    ...item,
                    description: art.descripcion || item.description,
                    idTablaCambioMedida: art.idTablaCambioMedida || 'S/N',
                    ritmo: art.ritmoTH || item.ritmo,
                    diasFabricacion: (art.ritmoTH || 0) > 0 ? (item.quantity / (art.ritmoTH || 1)) / 24 : item.diasFabricacion
                };
            }
            return item;
        });
        setDraftItems(refreshed);
        alert(`✅ Actualizados ${updated} de ${draftItems.length} ítems con el maestro de L${activeProcessId.replace('laminador', '')}.`);
    };

    const handleExportExcel = () => {
        if (!activeResult?.secuencia) return;

        const dataToExport = activeResult.secuencia.map((item: any, idx: number) => {
            const originalIdx = item.sku;
            const draftId = activeResult.params_ids[originalIdx];
            const draft = draftItems.find(d => d.id === draftId);
            const tc = (activeResult.tiemposCambio && activeResult.tiemposCambio[idx] > 0)
                ? activeResult.tiemposCambio[idx]
                : 0;

            return {
                'Orden': idx + 1,
                'SKU': activeResult.params_skus[originalIdx],
                'Descripción': activeResult.params_desc[originalIdx],
                'Cant. (TN)': activeResult.params_cant[originalIdx],
                'T. Cambio (H)': tc,
                'Stock Inicial': draft?.stockInicial || 0,
                'Venta Diaria': draft?.ventaDiaria || 0,
                'Días Stock': draft?.diasStock || 0,
                'Días Fab.': draft?.diasFabricacion || 0,
                'ID Cambio': draft?.idTablaCambioMedida || 'S/N'
            };
        });

        const ws = XLSX.utils.json_to_sheet(dataToExport);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Secuencia Optimizada");
        ...
        XLSX.writeFile(wb, `Secuencia_Optimizada_${activeProcessId}.xlsx`);
    };
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button onClick={handleRefreshDraft} disabled={draftItems.length === 0}
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold transition-all shadow-sm border text-sm ${draftItems.length === 0 ? 'text-gray-300 border-gray-100' : 'text-emerald-600 border-emerald-200 hover:bg-emerald-50'}`}
                            title="Re-mapear SKUs con el Maestro de Artículos del laminador activo">
                            <RefreshCw size={15} />
                            Sincronizar
                        </button>
                        <button onClick={() => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="border-l border-gray-100 pl-6 flex flex-col justify-center gap-2">
                        ...
                        <div className="flex items-center gap-3 text-sm">
                            <div className="w-32 text-gray-400 font-bold uppercase text-[10px]">Total TN Borrador:</div>
                            <div className="font-black text-orange-600">{draftItems.reduce((s, it) => s + it.quantity, 0).toLocaleString()} TN</div>
                        </div>
                        <div className="flex items-center gap-3 text-sm">
                ...
                isCalculating ? (
                    ...
                    <div className="space-y-6 pb-12 animate-in slide-in-from-bottom-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                            {/* KPI: Total TN comparador borrador vs resultado */}
                            <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden group">
                                <div className="absolute top-0 right-0 p-3 text-violet-50 group-hover:text-violet-100 transition-colors"><Package size={40} /></div>
                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Total TN</span>
                                <div className="text-xl font-black text-violet-700">
                                    {activeResult.params_cant
                                        ? activeResult.params_cant.reduce((a: number, b: number) => a + b, 0).toLocaleString()
                                        : '-'} TN
                                </div>
                                <div className="text-[9px] text-gray-400 mt-1">
                                    Borrador: <span className="font-bold text-orange-600">{draftItems.reduce((s, it) => s + it.quantity, 0).toLocaleString()} TN</span>
                                </div>
                            </div>
                            <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden group">
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                            <div className="px-5 py-3 bg-slate-50 border-b border-gray-200 flex justify-between items-center">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-[10px] font-black uppercase text-slate-600 hover:bg-slate-50 hover:text-blue-600 transition-all shadow-sm"
                                >
                                    <Download size={14} />
                                    Exportar Excel
                                </button>
                            </div>
                            <div className="overflow-x-auto overflow-y-auto max-h-[600px] scroll-smooth custom-scrollbar">
                                <table className="w-full text-left text-sm relative border-separate border-spacing-0">
                                    <thead className="sticky top-0 z-10 bg-slate-50 shadow-sm">
                                        <tr>
                                            ...
                                            <th className="px-6 py-3 border-b border-slate-200 text-right">Cant. (Tn)</th>
                                            <th className="px-6 py-3 border-b border-slate-200 text-right text-orange-600">T. Cambio (h)</th>
                                            <th className="px-6 py-3 border-b border-slate-200 text-right">Stock</th>
                                            <th className="px-6 py-3 border-b border-slate-200 text-right">Venta D.</th>
                                            <th className="px-6 py-3 border-b border-slate-200 text-right">Cobertura</th>
                                            <th className="px-6 py-3 border-b border-slate-200 text-right">Días Fab.</th>
                                            <th className="px-6 py-3 border-b border-slate-200">ID Cambio</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                            ...
                                            return (
                                                <tr key={idx} className="border-b border-slate-100 hover:bg-blue-50/30 transition-colors group">
                                                    ...
                                                    <td className="px-6 py-3 text-right font-mono font-bold text-orange-600 bg-orange-50/50">
                                                        {(activeResult.tiemposCambio && activeResult.tiemposCambio[idx] > 0)
                                                            ? activeResult.tiemposCambio[idx].toFixed(1)
                                                            : '-'}
                                                    </td>
